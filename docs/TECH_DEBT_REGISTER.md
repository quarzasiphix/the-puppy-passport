...
- Marketplace search (`/find-a-dog`) 100% client-side filtering, no server-side query — **fixed
  Stage IR-2** (`listPublishedPuppies()`/`countPublishedPuppies()` now filter and paginate
  server-side).
- `src/lib/supabase/types.ts` enum fidelity (plain `string` instead of real enum unions) — **fixed
  Stage IR-5**, which also retired the hand-written stub entirely in favour of real generated
  types via `npm run db:types`.
