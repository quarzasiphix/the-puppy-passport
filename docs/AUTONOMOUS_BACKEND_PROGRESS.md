...
**Original queue**: P (launch reconciliation), Q (final verification + report). (L, M, N and O are
complete — see table above.)

## Next up

Stage P (launch blocker cleanup, reconcile stale docs against real repo state). Re-run the Stage A
baseline checks (`git status`, `db reset`, `test:db` ×2, `tsc`, `build`) before starting, per the
