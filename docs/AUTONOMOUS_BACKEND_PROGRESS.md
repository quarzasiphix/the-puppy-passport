...
The first supplemental queue (Stage R onward — see "Supplemental queue appended mid-session"
above), per the standing instruction to continue directly without stopping at completing the
original queue. Re-run the Stage A baseline checks (`git status`, `db reset`, `test:db` ×2, `tsc`,
`build`) before starting, per the "how to resume" section below.
