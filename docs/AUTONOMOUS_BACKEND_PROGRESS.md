...
**Original queue**: Q (final verification + report). (L, M, N, O and P are complete — see table
above.)

## Next up

Stage Q (final verification — a 20-point checklist — plus `docs/BACKEND_FINALISATION_REPORT.md`).
Re-run the Stage A baseline checks (`git status`, `db reset`, `test:db` ×2, `tsc`, `build`) before
starting, per the "how to resume" section below. After Q, continue directly into the first
supplemental queue (Stage R onward — see "Supplemental queue appended mid-session" above), per the
standing instruction not to stop at completing the original queue.
