...
const statusLabel: Record<Puppy["status"], string> = {
  available: "Available",
    ...
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-border/70 bg-card transition-all hover:-translate-y-0.5 hover:shadow-lg">
      <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
        ...
        <button
          type="button"
          aria-label="Save"
          className="absolute right-3 top-3 grid size-9 place-items-center rounded-full bg-background/85 text-muted-foreground shadow-sm backdrop-blur hover:text-accent"
        >
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <div className="grid grid-cols-2 gap-1.5 text-xs text-muted-foreground">
          ...
          <span className="inline-flex items-center gap-1.5">
            <Calendar className="size-3.5" /> Ready {new Date(p.readyDate).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
          </span>
        </div>
    ...
    <article className="flex flex-col overflow-hidden rounded-2xl border border-border/70 bg-card">
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          <Meta label={planned ? "Expected birth" : "Born"} value={new Date(l.birthDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })} />
          <Meta label="Collection ready" value={new Date(l.readyDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })} />
          <Meta label="Mother" value={l.mother} />
          <Meta label="Father" value={l.father} />
          <Meta label="Breeder" value={l.kennel} />
          <Meta label={planned ? "Approx. places" : "Available"} value={`${l.available} / ${l.puppyCount}`} />
        </dl>
        {planned && (
        ...
        <div className="mt-auto flex gap-2 pt-2">
          ...
          {planned && <Button className="flex-1">Join waiting list</Button>}
        </div>
      </div>
    </article>
  );
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-right text-sm">
            <div className="font-medium">★ {b.rating.toFixed(1)}</div>
            <div className="text-xs text-muted-foreground">{b.reviewCount} reviews</div>
          </div>
        </div>
