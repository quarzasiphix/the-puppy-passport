...
/** Shared by /dashboard/admin/moderation and /dashboard/moderator/moderation — same table, same
 * review + enforcement actions, gated only by is_moderator() at the RPC layer (an admin is always
 * also a moderator per is_moderator()'s own definition). */
export function ModerationPanel() {
  ...
  // Enforcement actions -- what actually happens to the reported content, independent of the
  // paperwork above (a moderator may act immediately, then resolve the case with notes after).
  const enforce = useMutation({
    mutationFn: async (
      input:
        | { kind: "hide_post"; targetId: string; reason?: string }
        | { kind: "unhide_post"; targetId: string; reason?: string }
        | { kind: "unpublish_animal"; targetId: string; reason?: string }
        | { kind: "republish_animal"; targetId: string; reason?: string }
        | { kind: "suspend_org"; targetId: string; reason?: string }
        | { kind: "reinstate_org"; targetId: string; reason?: string },
    ) => {
      switch (input.kind) {
        case "hide_post":
          return setPostModerationStatus(input.targetId, "hidden", input.reason);
        case "unhide_post":
          return setPostModerationStatus(input.targetId, "visible", input.reason);
        case "unpublish_animal":
          return setAnimalPublished(input.targetId, false, input.reason);
        case "republish_animal":
          return setAnimalPublished(input.targetId, true, input.reason);
        case "suspend_org":
          return setOrganisationSuspended(input.targetId, true, input.reason);
        case "reinstate_org":
    ...
    onError: (err) => toast.error(err instanceof Error ? err.message : "Could not apply action."),
  });
        ...
        <div className="space-y-3">
          {query.data.map((c) => (
            <div key={c.id} className="rounded-2xl border border-border/70 bg-card p-5">
              ...
              {/* Enforcement — takes effect immediately, independent of the case's own
                  investigating/resolved paperwork status below. */}
              {c.target_type === "post" && (
                <div className="mt-3 flex gap-2">
                  <Button
                    ...
                    onClick={() => enforce.mutate({ kind: "hide_post", targetId: c.target_id })}
                  >
                    <EyeOff className="mr-1 size-4" /> Hide post
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={enforce.isPending}
                    onClick={() => enforce.mutate({ kind: "unhide_post", targetId: c.target_id })}
                  >
                    <Eye className="mr-1 size-4" /> Unhide post
                  </Button>
                </div>
              )}
              {c.target_type === "animal_listing" && (
                <div className="mt-3 flex gap-2">
                  <Button
                    ...
                    disabled={enforce.isPending}
                    onClick={() =>
                      enforce.mutate({ kind: "unpublish_animal", targetId: c.target_id })
                    }
                  >
                    <EyeOff className="mr-1 size-4" /> Unpublish listing
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={enforce.isPending}
                    onClick={() =>
                      enforce.mutate({ kind: "republish_animal", targetId: c.target_id })
                    }
                  >
                    <Eye className="mr-1 size-4" /> Republish listing
                  </Button>
                </div>
              )}
              {c.target_type === "organisation" && (
                <div className="mt-3 flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    ...
                    disabled={enforce.isPending}
                    onClick={() => enforce.mutate({ kind: "suspend_org", targetId: c.target_id })}
                  >
                    <Ban className="mr-1 size-4" /> Suspend organisation
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={enforce.isPending}
                    onClick={() => enforce.mutate({ kind: "reinstate_org", targetId: c.target_id })}
                  >
                    <CheckCircle2 className="mr-1 size-4" /> Reinstate organisation
                  </Button>
                </div>
              )}
