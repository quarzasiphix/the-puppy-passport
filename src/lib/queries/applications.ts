...
export async function respondToApplication(params: {
  ...
  const { error } = await supabase
    .from("buyer_applications")
    .update({ status: params.status, breeder_response: params.breederResponse })
    .eq("id", params.id);
  if (error) throw error;
  await notifyUser({
