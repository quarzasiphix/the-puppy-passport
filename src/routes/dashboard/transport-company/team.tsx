...
export const Route = createFileRoute("/dashboard/transport-company/team")({
  component: TeamPage,
});

// A company owner invites operational staff — not mobile freelance drivers (that's the
// individual /dashboard/driver workspace, unrelated to org membership at all).
const invitableRoles: OrgMemberRole[] = ["administrator", "transport_coordinator", "employee"];
...
function TeamPage() {
  ...
  const inviteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("transportCompanyPanel.team.invitedToast"));
      setOpen(false);
      setEmail("");
      setRole("employee");
      invalidate();
    ...
    onError: (err) =>
      toast.error(getFriendlyErrorMessage(err, t("transportCompanyPanel.team.inviteFailed"))),
  });
  ...
  const revokeMutation = useMutation({
    ...
    onError: (err) =>
      toast.error(getFriendlyErrorMessage(err, t("transportCompanyPanel.team.revokeFailed"))),
  });

  const removeMutation = useMutation({
    mutationFn: (id: string) => removeOrgMember(id),
    onSuccess: () => {
      toast.success(t("transportCompanyPanel.team.removedToast"));
      invalidate();
    },
    onError: (err) =>
      toast.error(getFriendlyErrorMessage(err, t("transportCompanyPanel.team.removeFailed"))),
  });
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: invalidate,
    onError: (err) =>
      toast.error(getFriendlyErrorMessage(err, t("transportCompanyPanel.team.statusUpdateFailed"))),
  });
  ...
  const roleMutation = useMutation({
    ...
    onSuccess: invalidate,
    onError: (err) =>
      toast.error(getFriendlyErrorMessage(err, t("transportCompanyPanel.team.roleChangeFailed"))),
  });
  ...
  return (
    <div>
      <header className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-medium">
            {t("transportCompanyPanel.team.title")}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("transportCompanyPanel.team.subtitle")}
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button size="sm" disabled={!orgQuery.data?.id}>
              <Plus className="mr-1 size-4" /> {t("transportCompanyPanel.team.inviteButton")}
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t("transportCompanyPanel.team.inviteDialogTitle")}</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              <div>
                <Label>{t("transportCompanyPanel.team.fieldEmail")}</Label>
                <Input
                  type="email"
                  value={email}
              ...
              <div>
                <Label>{t("transportCompanyPanel.team.fieldRole")}</Label>
              ...
              >
                {t("transportCompanyPanel.team.sendInvitationButton")}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
      ...
      {!orgQuery.isLoading && !orgQuery.data && (
        <div className="rounded-2xl border border-dashed border-border/70 bg-secondary/40 p-8 text-center">
          <p className="text-sm text-muted-foreground">
            {t("transportCompanyPanel.team.notAvailableNote")}
          </p>
        </div>
      )}
      ...
      {!!pendingInvitations.length && (
        <section className="mb-6">
          <h2 className="mb-3 font-display text-lg font-semibold">
            {t("transportCompanyPanel.team.pendingInvitationsTitle")}
          </h2>
          <div className="space-y-2">
            {pendingInvitations.map((inv) => (
              ...
              >
                ...
                >
                  {t("transportCompanyPanel.team.revokeButton")}
                </Button>
              </div>
            ))}
          </div>
      ...
      <section>
        <h2 className="mb-3 font-display text-lg font-semibold">
          {t("transportCompanyPanel.team.membersTitle")}
        </h2>
        <div className="space-y-2">
            ...
            >
              <div>
                <div className="text-sm font-medium">
                  {m.profiles?.display_name ?? t("transportCompanyPanel.team.defaultMemberName")}
                </div>
                <div className="text-xs text-muted-foreground">
                  {orgMemberRoleLabels[m.member_role]}
                  {m.status === "suspended" &&
                    ` · ${t("transportCompanyPanel.team.suspendedSuffix")}`}
                </div>
              </div>
                ...
                {m.member_role !== "owner" && (
                  <>
                    ...
                    >
                      ...
                      {m.status === "active"
                        ? t("transportCompanyPanel.team.suspendButton")
                        : t("transportCompanyPanel.team.restoreButton")}
                    </Button>
                    <Button
                      size="sm"
                      variant="ghost"
                    ...
                    >
                      ...
                      {t("transportCompanyPanel.team.removeButton")}
                    </Button>
                  </>
                )}
              </div>
